...
Payload (JSON): {"q": "<quarantine-uuid>", "a": "<approve|reject>", "e": <expiry>}
...
async def make_token(session: AsyncSession, quarantine_id: UUID,
    ...
    """Create a signed token for a quarantine action."""
...
async def verify_token(session: AsyncSession, token: str) -> tuple[UUID, str]:
    """Verify a token and return (quarantine_id, action).
    ...
    if not all(k in payload for k in ("q", "a", "e")):
        raise ValueError("payload missing fields")
    if payload["a"] not in ("approve", "reject"):
        raise ValueError("invalid action")
    if int(payload["e"]) < int(time.time()):
        raise ValueError("token expired")

    return UUID(payload["q"]), payload["a"]
